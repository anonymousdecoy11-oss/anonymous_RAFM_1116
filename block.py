...
class RAFM(nn.Module):
    """
    Routing-Aware Frequency Module.

    PCB traces route at 0, 45, 90, 135 degrees. Every defect class
    manifests as an anomaly relative to one of these routing directions.
    Standard isotropic convolutions treat all directions equally and
    cannot distinguish a scratch-along-trace from a scratch-across-trace.

    RAFM decomposes features into 4 directional components using
    oriented filters initialised at PCB routing angles, then learns
    which directional anomalies correspond to which defect types.

    Parameters increase is justified because:
    - 4 directional filters replace 1 isotropic filter
    - Each direction captures a geometrically distinct defect signature
    - A generic conv of the same size cannot learn this decomposition
      because it has no inductive bias toward routing angles
    """
    def __init__(self, c1):
        super().__init__()
        c_ = max(c1 // 4, 16)

        # ── Four directional filter banks (one per routing angle) ─────────
        # Depthwise so each channel gets its own directional decomposition
        # Initialised at 0°, 45°, 90°, 135° — learns PCB-specific refinement
        self.dir_filters = nn.ModuleList([
            nn.Conv2d(c1, c1, 3, 1, 1, groups=c1, bias=False)
            for _ in range(4)
        ])
        self._init_directional_filters()

        # ── Directional anomaly scorer ─────────────────────────────────────
        # For each direction, how anomalous is the response?
        # High anomaly = defect present in this routing direction
        self.anomaly_score = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(c1, c_, 1),
                nn.BatchNorm2d(c_),
                nn.ReLU(inplace=True),
                nn.Conv2d(c_, 1, 1),
                nn.Sigmoid()              # (B, 1, H, W) per direction
            ) for _ in range(4)
        ])

        # ── Cross-direction attention ──────────────────────────────────────
        # Learns which directions matter for which defect class
        # e.g. bridge defects need both horizontal AND vertical attention
        self.cross_dir = nn.Sequential(
            nn.Conv2d(4, 4, 1),           # 4 direction scores → mix
            nn.Softmax(dim=1),            # normalise across directions
        )
    ...
    def _init_directional_filters(self):
        """
        Initialise 4 directional filters at PCB routing angles.

        0°   (horizontal):  [ 1  2  1]   Detects horizontal trace responses
                             [ 0  0  0]
                             [-1 -2 -1]

        90°  (vertical):    [ 1  0 -1]   Detects vertical trace responses
                             [ 2  0 -2]
                             [ 1  0 -1]

        45°  (diagonal):    [ 0  1  2]   Detects 45° diagonal traces
                             [-1  0  1]
                             [-2 -1  0]

        135° (antidiag):    [ 2  1  0]   Detects 135° diagonal traces
                             [ 1  0 -1]
                             [ 0 -1 -2]
        """
        kernels = [
            torch.tensor([[ 1.,  2.,  1.],   # 0°
                          [ 0.,  0.,  0.],
                          [-1., -2., -1.]]),
            torch.tensor([[ 1.,  0., -1.],   # 90°
                          [ 2.,  0., -2.],
                          [ 1.,  0., -1.]]),
            torch.tensor([[ 0.,  1.,  2.],   # 45°
                          [-1.,  0.,  1.],
                          [-2., -1.,  0.]]),
            torch.tensor([[ 2.,  1.,  0.],   # 135°
                          [ 1.,  0., -1.],
                          [ 0., -1., -2.]]),
        ]
        with torch.no_grad():
            for d, (filt, kernel) in enumerate(
                    zip(self.dir_filters, kernels)):
                for i in range(filt.weight.shape[0]):
                    filt.weight[i, 0] = kernel

    def forward(self, x):
        B, C, H, W = x.shape

        # ── Step 1: Compute directional responses ─────────────────────────
        dir_responses = []
        for d in range(4):
            resp = self.dir_filters[d](x)      # (B, C, H, W)
            resp = torch.abs(resp)             # magnitude only
            dir_responses.append(resp)

        # ── Step 2: Score anomaly per direction ───────────────────────────
        # High score = unexpected response in this routing direction = defect
        scores = []
        for d in range(4):
            s = self.anomaly_score[d](dir_responses[d])  # (B, 1, H, W)
            scores.append(s)

        # ── Step 3: Cross-direction weighting ────────────────────────────
        # Stack scores: (B, 4, H, W)
        score_map = torch.cat(scores, dim=1)
        # Learn which direction combo matters for each spatial location
        dir_weights = self.cross_dir(score_map)  # (B, 4, H, W) softmax

        # ── Step 4: Weight directional features by anomaly score ──────────
        weighted = []
        for d in range(4):
            w = dir_weights[:, d:d+1, :, :]    # (B, 1, H, W)
            weighted.append(dir_responses[d] * w)

        # ── Step 5: Reconstruct to original channel count ─────────────────
        fused = self.reconstruct(
            torch.cat(weighted, dim=1)          # (B, C*4, H, W)
        )

        # ── Step 6: Gated residual ────────────────────────────────────────
        return self.bn(x + torch.tanh(self.gate) * fused)
class DFL(nn.Module):
...
class MSFA(nn.Module):
    ...
    Runs 3 parallel dilated depthwise convolutions (r=1,2,4) then
    uses squeeze-excitation to weight which scale matters per channel.
    """
    def __init__(self, c1):
        super().__init__()
        c_ = max(c1 // 4, 16)

        # three parallel dilated depthwise branches
        self.br1 = nn.Sequential(
            nn.Conv2d(c1, c1, 3, 1, 1,  dilation=1, groups=c1, bias=False),
            nn.BatchNorm2d(c1), nn.ReLU(inplace=True))
        self.br2 = nn.Sequential(
            nn.Conv2d(c1, c1, 3, 1, 2,  dilation=2, groups=c1, bias=False),
            nn.BatchNorm2d(c1), nn.ReLU(inplace=True))
        self.br4 = nn.Sequential(
            nn.Conv2d(c1, c1, 3, 1, 4,  dilation=4, groups=c1, bias=False),
            nn.BatchNorm2d(c1), nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        b2  = self.br2(x)
        b4  = self.br4(x)
        out = self.fuse(torch.cat([b1, b2, b4], dim=1))
        w   = self.se(out).view(x.shape[0], x.shape[1], 1, 1)
        out = out * w
        return self.bn(x + torch.tanh(self.gate) * out)
